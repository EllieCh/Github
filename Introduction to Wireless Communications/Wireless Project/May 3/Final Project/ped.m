...
%Pedestrian Channel Co-efficients

c0_p = (10^0).*randn(100,1);  %0 nsec
c1_p = (10^-0.97).*randn(100,1);      %110 nsec
c2_p = (10^-1.92).*randn(100,1);  % 190 nsec
c3_p = (10^-2.28).*randn(100,1);     %410 nsec


%Delays Associated with Pedestrian Channel

dped(:,0/ts +1) =   c0_p;
dped(:,int16(0.11/ts)) = c1_p;
dped(:,int16(0.19/ts)) = c2_p;
dped(:,int16(0.41/ts)) = c3_p;
...
title('BER vs SNR for Pedestrian Channel')
xlabel('SNR (db)')
ylabel('BER')
grid on
...
y_p = cconv(dped(n,:),sig,N+cp);
noisy_y_p = awgn(y_p,snr,'measured');
chan_out = noisy_y_p;
