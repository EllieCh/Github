...
for sym = 1:totsymbol
    ...
    for i = 1:datacarr
            ...
            if (bit(x) == 0 && bit(y) == 0 && bit(z) == 0)
                ang = -3*pi/4; 
            elseif (bit(x) == 0 && bit(y) == 0 && bit(z) == 1)
                ang = pi; 
            elseif (bit(x) == 0 && bit(y) == 1 && bit(z) == 0)
                ang = pi/2; 
            elseif (bit(x) == 0 && bit(y) == 1 && bit(z) == 1)    
                ang = 3*pi/4;     
            elseif (bit(x) == 1 && bit(y) == 0 && bit(z) == 0)
                ang = -pi/2; 
            elseif (bit(x) == 1 && bit(y) == 0 && bit(z) == 1)
                ang = -pi/4;
             elseif (bit(x) == 1 && bit(y) == 1 && bit(z) == 0)
                ang = pi/4;     
            elseif (bit(x) == 1 && bit(y) == 1 && bit(z) == 1)
                ang = 0;
...
subplot(2,1,1);
plot(real(txsymbit));
title('Real Part of OFDM Symbols using 8-PSK')
xlabel('Sub-Carriers');
ylabel('Magnitude');
subplot(2,1,2);
plot(imag(txsymbit));
title('Imaginary Part of OFDM Symbols using 8-PSK')
xlabel('Sub-Carriers');
ylabel('Magnitude');
figure;
freqz(ofdmfreq);
