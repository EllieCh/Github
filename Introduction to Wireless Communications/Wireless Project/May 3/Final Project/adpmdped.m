...
c2=sqrt(0.107)*(randn(1,size)+(sqrt(-1)*randn(1,size))); 
c3=sqrt(0.012)*(randn(1,size)+(sqrt(-1)*randn(1,size))); 
c4=sqrt(0.00524)*(randn(1,size)+(sqrt(-1)*randn(1,size))); 
...
plot(snr_db,Spectral_Efficency,'-+g'),
title('Spectral Efficieny for Pedestrian Channnel using Adaptive Modulation ');
xlabel('SNR');
