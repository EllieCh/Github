...
for sym = 1:totsymbol
    ...
    for i = 1:datacarr
            ...
            if (bit(z) == 0 && bit(u) == 0)
                imqam = 3;
            elseif (bit(z) == 0 && bit(u) == 1)
                imqam = 1;
            elseif (bit(z) == 1 && bit(u) == 0)
                imqam = -3;    
            elseif (bit(z) == 1 && bit(u) == 1)
                imqam = -1;
            
            end
            
            bit1(i) = complex(reqam,imqam);
            Es = 10;
...
subplot(2,1,1);
plot(real(txsymbit));
title('Real Part of OFDM Symbols using 16-QAM')
xlabel('Sub-Carriers');
ylabel('Magnitude');
subplot(2,1,2);
plot(imag(txsymbit));
title('Imaginary Part of OFDM Symbols using 16-QAM')
xlabel('Sub-Carriers');
ylabel('Magnitude');
figure;
freqz(ofdmfreq);
