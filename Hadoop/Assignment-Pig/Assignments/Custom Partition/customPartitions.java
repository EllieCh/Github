...
public class customPartition  {
		...
		//In this example, there are 3 partitions, the first partition contains the information where the length of name is less than 2
		//The second partition contains data with length of name ranging between 2 and 5 and the third partition contains data where the length of name is >5.
		public static class AgePartitioner extends Partitioner<Text, Text> {
			...
			public int getPartition(Text key, Text value, int numReduceTasks) {
				...
				String age = nameAgeScore[0];
				//int ageInt = Integer.parseInt(age);
				
				int name = age.length();

				//if the length of name is <2, assign partition 0
				if(name < 2){				
					return 0;
				}
				//else if the length of name is between 2 and 5, assign partition 1
				if(name > 2 && name <=5){
